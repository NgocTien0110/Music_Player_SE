...
const Lyric = () => {
    ...
    const handleCloseLyric = () => {
        isLyric ? dispatch(setOpenLyric(false)) : dispatch(setOpenLyric(true));
    };
    ...
    return (
        <>
            <div
                className={
                    'fixed inset-0 z-[200] bg-slate-900 transition-all ease-linear duration-300 ' +
                        ...
                        lyric.map((e) => {
                            return (
                                <div className="my-[2px] mx-0 px-[18px] py-3 rounded-xl hover:bg-[color:var(--color-secondary-bg-for-transparent)] box-border">
                                    <span className="cursor-pointer inline-block opacity-90">{e.data}</span>
                                </div>
                            );
                        })}
                    {/* End Line Lyric*/}
